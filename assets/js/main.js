...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (curriculumConfigEl && curriculumChartEl) {
    ...
    const setActiveProfile = (key) => {
      ...
      curriculumButtons.forEach((btn) => {
        ...
        if (isActive) {
          btn.classList.add('bg-wimper-blue', 'text-white', 'shadow-soft');
          btn.classList.remove('text-brand-ink/70', 'bg-white');
        } else {
          btn.classList.remove('bg-wimper-blue', 'text-white', 'shadow-soft');
  ...
  if (schedule) {
    ...
    const activate = (key) => {
      tabs.forEach((btn) => {
        ...
        btn.classList.toggle('bg-wimper-blue', isActive);
  ...
  if (reviewsCarousel) {
    ...
    const goToSlide = (index) => {
      ...
      dots.forEach((dot, i) => {
        if (i === currentIndex) {
          dot.classList.remove('bg-wimper-blue/30', 'hover:bg-wimper-blue/50', 'w-3');
          dot.classList.add('bg-wimper-red', 'w-8');
        } else {
          dot.classList.remove('bg-wimper-red', 'w-8');
          dot.classList.add('bg-wimper-blue/30', 'hover:bg-wimper-blue/50', 'w-3');
        }
      });
    };

  ...
  <header class="sticky top-0 z-50 bg-white/90 backdrop-blur-xl border-b border-chroma-blue/10">
    <div class="max-w-7xl mx-auto px-4 lg:px-6 h-[82px] flex items-center justify-between">
      ...
      <nav class="hidden md:flex items-center gap-8 text-sm font-semibold text-brand-ink/70">
        ...
        if ( $stories_page ) :
        ?>
        <a href="<?php echo esc_url( get_permalink( $stories_page->ID ) ); ?>" class="hover:text-chroma-blue">Stories (Blog)</a>
        <?php endif; ?>
        <?php
        $contact_page = get_page_by_path( 'contact' );
        if ( $contact_page ) :
        ?>
        <a href="<?php echo esc_url( get_permalink( $contact_page->ID ) ); ?>" class="hover:text-chroma-blue">Media Contact</a>
        <?php endif; ?>
      </nav>
    </div>
  </header>
  ...
  <main>
    ...
    <section class="py-16 max-w-5xl mx-auto px-4">
      ...
      <div class="space-y-12">
        <?php
        $post_count = 0;
        while ( $newsroom_query->have_posts() ) : $newsroom_query->the_post();
          if ( $post_count > 0 ) : ?>
          ...
          <div class="group">
            ...
            <a href="<?php the_permalink(); ?>" class="text-sm font-bold border-b-2 border-chroma-yellow pb-0.5 hover:text-chroma-blue hover:border-chroma-blue transition-colors">
              Read Release
            </a>
          </div>
    ...
    <section class="py-16 bg-brand-ink text-white text-center">
      <div class="max-w-2xl mx-auto px-4">
        ...
        ?>
        <a href="<?php echo esc_url( $contact_url ); ?>" class="inline-block px-8 py-3 bg-white text-brand-ink font-bold rounded-full text-xs uppercase tracking-widest hover:bg-chroma-yellow transition-colors">
          Contact Media Team
        </a>
      </div>
    </section>

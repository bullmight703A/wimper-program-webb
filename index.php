			...
			<?php while (have_posts()):
				...
				<article id="post-<?php the_ID(); ?>" <?php post_class('bg-white rounded-3xl p-8 shadow-card'); ?>>
					<h2 class="text-2xl font-serif font-bold text-brand-ink mb-4">
						<a href="<?php the_permalink(); ?>" class="hover:text-wimper-teal">
							<?php the_title(); ?>
						</a>
					</h2>
					...
					<a href="<?php the_permalink(); ?>"
						class="inline-flex items-center gap-2 mt-4 text-wimper-teal font-semibold hover:text-wimper-blueDark">
						Read more →
					</a>
				</article>
			<?php endwhile; ?>
